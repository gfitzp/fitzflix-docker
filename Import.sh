...
then
	...
	mysql -h ${MYSQL_HOST} -P ${MYSQL_PORT:=3306} -u ${MYSQL_USER} -p${MYSQL_PASSWORD} ${MYSQL_DB:="fitzflix_db"} -e "INSERT INTO presets_titles (plex_name, movie_title, release_year, release_identifier) VALUES ('${escaped_plex_name}', '${escaped_movie_title}', '${escaped_release_year}', ${escaped_release_identifier});"
...
then
	...
	# Add the file to the database
	mysql -h ${MYSQL_HOST} -P ${MYSQL_PORT:=3306} -u ${MYSQL_USER} -p${MYSQL_PASSWORD} ${MYSQL_DB:="fitzflix_db"} -e "INSERT INTO presets_series (series_title) VALUES ('${escaped_series_title}');"
