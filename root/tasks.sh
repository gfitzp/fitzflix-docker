...
archive_video () {
	...
	mysql -h ${MYSQL_HOST:-${MYSQL_PORT_3306_TCP_ADDR}} -P ${MYSQL_PORT:-${MYSQL_PORT_3306_TCP_PORT:=3306}} -u ${MYSQL_USER} -p${MYSQL_PASSWORD} ${MYSQL_DB:="fitzflix_db"} -e "UPDATE files SET date_file_archived = CURRENT_TIMESTAMP WHERE file_path = '${escaped_file_path}';"
...
delete_video() {
	...
	mysql -h ${MYSQL_HOST:-${MYSQL_PORT_3306_TCP_ADDR}} -P ${MYSQL_PORT:-${MYSQL_PORT_3306_TCP_PORT:=3306}} -u ${MYSQL_USER} -p${MYSQL_PASSWORD} ${MYSQL_DB:="fitzflix_db"} -e "INSERT INTO history_task (queue_start, file_path, task, task_duration) SELECT FROM_UNIXTIME('${queueStart}'), file_path, task, '${task_duration}' FROM v_queue WHERE file_path = '${escaped_file_path}';" &&
	mysql -h ${MYSQL_HOST:-${MYSQL_PORT_3306_TCP_ADDR}} -P ${MYSQL_PORT:-${MYSQL_PORT_3306_TCP_PORT:=3306}} -u ${MYSQL_USER} -p${MYSQL_PASSWORD} ${MYSQL_DB:="fitzflix_db"} -e "UPDATE files SET date_file_deleted = CURRENT_TIMESTAMP WHERE file_path = '${escaped_file_path}';"
...
restore_video () {
	...
	mysql -h ${MYSQL_HOST:-${MYSQL_PORT_3306_TCP_ADDR}} -P ${MYSQL_PORT:-${MYSQL_PORT_3306_TCP_PORT:=3306}} -u ${MYSQL_USER} -p${MYSQL_PASSWORD} ${MYSQL_DB:="fitzflix_db"} -e "INSERT INTO history_task (queue_start, file_path, task, task_duration) SELECT FROM_UNIXTIME('${queueStart}'), file_path, task, '${task_duration}' FROM v_queue WHERE file_path = '${escaped_file_path}';" &&
	mysql -h ${MYSQL_HOST:-${MYSQL_PORT_3306_TCP_ADDR}} -P ${MYSQL_PORT:-${MYSQL_PORT_3306_TCP_PORT:=3306}} -u ${MYSQL_USER} -p${MYSQL_PASSWORD} ${MYSQL_DB:="fitzflix_db"} -e "UPDATE files SET date_restore_requested = CURRENT_TIMESTAMP WHERE file_path = '${escaped_file_path}';"
...
encode_video () {
	...
	if [[ "${task}" == "encode" ]]
	then
		mysql -h ${MYSQL_HOST:-${MYSQL_PORT_3306_TCP_ADDR}} -P ${MYSQL_PORT:-${MYSQL_PORT_3306_TCP_PORT:=3306}} -u ${MYSQL_USER} -p${MYSQL_PASSWORD} ${MYSQL_DB:="fitzflix_db"} -e "UPDATE presets_titles SET latest_transcode = CURRENT_TIMESTAMP WHERE plex_name = '${escaped_plex_name}';"
	fi &&
...
purge_video () {
	...
	mysql -h ${MYSQL_HOST:-${MYSQL_PORT_3306_TCP_ADDR}} -P ${MYSQL_PORT:-${MYSQL_PORT_3306_TCP_PORT:=3306}} -u ${MYSQL_USER} -p${MYSQL_PASSWORD} ${MYSQL_DB:="fitzflix_db"} -e "INSERT INTO history_task (queue_start, file_path, task, task_duration) SELECT FROM_UNIXTIME('${queueStart}'), file_path, task, '${task_duration}' FROM v_queue WHERE file_path = '${escaped_file_path}';" &&
	mysql -h ${MYSQL_HOST:-${MYSQL_PORT_3306_TCP_ADDR}} -P ${MYSQL_PORT:-${MYSQL_PORT_3306_TCP_PORT:=3306}} -u ${MYSQL_USER} -p${MYSQL_PASSWORD} ${MYSQL_DB:="fitzflix_db"} -e "DELETE FROM files WHERE file_path = '${escaped_file_path}'; DELETE FROM presets_titles WHERE plex_name = '${escaped_plex_name}'; DELETE FROM presets_titles WHERE series_title = '${escaped_series_title}';"
